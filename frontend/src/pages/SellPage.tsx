            ...
            {STEPS.map((s, index) => (
              <div key={s.id} className="flex items-center">
                <div className={`
                  ...
                  ${step >= s.id ? 'bg-[#007AFF] text-white shadow-lg ring-4 ring-white' : 'bg-surface-container-highest text-secondary border-4 border-white'}
                `}>
                  {step > s.id ? (
                ...
                <span className={`ml-3 font-medium hidden sm:inline ${step >= s.id ? 'text-primary font-bold' : 'text-secondary'}`}>
                  {s.title}
                </span>
                {index < STEPS.length - 1 && (
                  <div className={`w-12 sm:w-16 h-1 mx-2 sm:mx-4 ${step > s.id ? 'bg-[#007AFF]' : 'bg-surface-container-highest'}`} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="bg-surface-container-lowest rounded-2xl shadow-[0_20px_40px_rgba(25,28,30,0.06)] p-8">
          {step === 1 && (
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-2">Marca *</label>
                  <input
                    type="text"
                    value={formData.brand}
                    onChange={(e) => updateForm('brand', e.target.value)}
                    placeholder="Ej: Toyota"
                    className="w-full bg-surface-container-low border-none rounded-lg py-3 px-4 focus:ring-2 focus:ring-[#007AFF]/20"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-2">Modelo *</label>
                  <input
                    type="text"
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-2">Año *</label>
                  <input
                    type="number"
                    value={formData.year}
                ...
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-2">Kilometraje *</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-2">Tipo de vehículo *</label>
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-2">Tu precio (USD) *</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant text-lg font-semibold">$</span>
                  <input
                    type="number"
              ...
              <div className="flex items-center gap-3 p-4 bg-surface-container-low rounded-lg">
                ...
                />
                <label htmlFor="negotiable" className="text-on-surface">
                  <span className="font-medium">Precio negociable</span>
                  <p className="text-sm text-on-surface-variant">Los compradores sabrán que estás abierto a ofertas</p>
                </label>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-2">Combustible *</label>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {FUEL_TYPES.map((type) => (
                    <button
                      key={type.value}
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-2">Transmisión *</label>
          ...
          {step === 4 && result && (
            <div className="text-center py-8">
              ...
              <div className="bg-surface-container-low rounded-xl p-6 mb-8">
                <p className="text-sm text-on-surface-variant mb-1">{result.brand} {result.model}</p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <button
                  ...
                  className="border border-surface-container-high px-6 py-3 rounded-lg font-medium text-on-surface hover:bg-surface-container-low"
                >
                  Mis vehículos
                </button>
              </div>
            ...
            <div className="flex justify-between mt-8 pt-6 border-t">
              ...
              {step < 3 ? (
                <button
                  onClick={() => setStep(s => s + 1)}
                  disabled={!canProceed()}
                  className="px-8 py-3 bg-[#007AFF] text-white font-bold rounded-lg shadow-md disabled:opacity-50 disabled:cursor-not-allowed hover:translate-y-[-1px] active:scale-95 transition-all"
                >
                  Siguiente
                </button>
              ) : (
